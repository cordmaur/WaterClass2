...
def get_calibrated_measurements(mdb, r_type='Ed', method_type=None, r_type_column='Comment',
                                r_types=None, with_reflectance=False, station=None):
    """Get the measurements only that have Ld, Lu e Ed and calibrated values"""
    if r_types is None:
        r_types = ['Ed', 'Ld', 'Lu']

    dates = get_calibrated_dates(mdb, r_type_column=r_type_column, r_types=r_types,
                                 with_reflectance=with_reflectance, station=station)

    idx = pd.DatetimeIndex(dates)
    df = pd.DataFrame(index=idx[~idx.duplicated()])
...
def create_radiometry_csv(mdb, r_type='Ed', r_type_column='Comment', method_type='SAM Control',
                          with_reflectance=False, times=None, station=None):
    """Create a csv file in the same format as the .MLB or the .TXT of the MSDA_XE software"""

    # get the measurements in the MDB matching r_type (Ed, Ld, Lu)
    measures = get_calibrated_measurements(mdb, r_type=r_type, r_type_column=r_type_column, method_type=method_type,
                                           with_reflectance=with_reflectance, station=station)
...
def export_mdb_csv2(mdb_path, r_types, out_dir=None, create_measurement_dir=True, with_reflectance=False,
                    times=None, station=None):
    ...
    if create_measurement_dir:
        if times is not None:
            ...
            dt = get_calibrated_dates(mdb_path, r_type_column=next(iter(r_types.values()))['column'],
                                      r_types=[item['descr'] for item in r_types.values()])[0]
    ...
    for r_type, values in r_types.items():
        txt = create_radiometry_csv(mdb_path,
                                    r_type=values['descr'],
                                    method_type=values['method'],
                                    r_type_column=values['column'],
                                    with_reflectance=with_reflectance,
                                    times=times,
                                    station=station)
...
class RadiometryDB:
    ...
    config = {
        ...
        "main_workbook": 'GETDB-Summary',
        "stations_workbook": 'Stations',
        "summertime_workbook": 'SummerTime',
    ...
    def __init__(self, path, config=None):
        ...
        # load main spreadsheets
        self.data, self.stations, self.summertime = self.load_control_file(self.path, self.config)
    ...
    # Loading functions
    @staticmethod
    def load_control_file(path, config):
    ...
    def load_radiometry(self, area, station, measurement, load_interpolated=True, read_backup=False):
    ...
    def load_summary(self, r_types=None, attrs=None):
        attrs = ['Area', 'Station', 'Lat', 'Long', 'GMT', 'SummerTime', 'OBS',
                 'Relative_Path', 'Start_Date', 'End_Date', 'Project',
                 'Description'] if attrs is None else listify(attrs)
        ...
        for r_type in r_types:
            self.rdmtries[r_type] = self.rdmtries[r_type].join(self.data[attrs], rsuffix='_2')
    ...
    def create_adjusted_reflectance(self, trios_main, filters=None):
        ...
        self.loop_database(funcs=map_adjusted_reflectance, filters=filters)
    ...
    def commit_workbook(self, workbook='main_workbook'):
        if workbook == 'stations_workbook':
            ...
            df = self.data[['Relative_Path', 'Station_Name', 'Measurement', 'SPM', 'POC', 'DOC', 'Status', 'Start_Date',
                           'End_Date', 'gmt_datetime', 'Project', 'Ed_device', 'Ld_device',
